C-------------------------------------------------------------
      SUBROUTINE NOVRLP(X,N,NP,NP1,NDIR,MA,RESID,JRES,JSAMP,
     f                   T,R,XN,EVECS,EVALS,AVE,COV,RLOC,RSCA,XA,
     f                   COEFFS,NSIN,NQUASI,
     f                   JLV,JRV,ETAS,Y,M,YS,MS,INDEX,F,S)
C  This program computes an approximation for n_overlap of
...
C  the reduced data set has no overlap)
C  Missing values are not allowed.       
     ...
     f        NNP,NDEP,NSIN
      ...
      INTEGER RESID(N),JRES(N),JSAMP(NP),NQUASI
      INTEGER j
...
C.... standardize XA
  ...
  20  FORMAT(/,' The standardization is completed.')
      NNP=NP
      DO 25 i=1,N
        Index(i)=0
  25  CONTINUE      
...
CCC         WRITE(lunout,218)
 ...
 216  format(/' The final overlap estimate is: ',i8,/)
...
CCC               WRITE(*,250)J
 ...
 10   CONTINUE
      RETURN
 100  FORMAT(' VARIABLE ',I4,' HAS ZERO VARIANCE, ')
 150  FORMAT(' DIMENSION REDUCED TO ',I4,'.')
 200  FORMAT(' AT LEAST HALF OF THE POINTS HAVE THE SAME ')
 250  FORMAT(' VALUE FOR VARIABLE ',I4,'.')
      END
...
C
  ...
  10  CONTINUE
      ...
      NRUN=0
      DO 100 NRAN=1,NDIR
CCC         IF ( (NRAN/1000)*1000-NRAN .EQ. 0 ) THEN
CCCCC        WRITE(*,807) NRAN   
...
C
         ...
         ENDIF
C
C  Compute the one-dimensional overlap of the fit,
C  and update the NP-dimensional depth.
C
C.......  Begin of the modification to check for overlap
...
C  for the reduced data set in a logistic regression model
         CALL overlp(Ntot,YS,MS,NS,NNEW,F,S)
...
CCC               WRITE(lunout,505) J,COEFFS(j)
 135           CONTINUE
            NDEP=NNEW
CCCCC            CALL INTPR('New NOVERLAP after ... of subsamples drawn',42,
CCCCC     f                 NRAN,1)
CCCCC            CALL INTPR('NOVERLAP',8,NDEP,1)
...
CCC              WRITE(lunout,840) index(Li),XN(Li),Y(Li),M(Li)
 ...
 500  format(/, 70('-'),/,
     ...
     +       ' , the program obtained overlap =',i8)
 ...
 807  FORMAT('--> Number of subsamples drawn',I8)    
 810  FORMAT ( ' Data after one dimensional aggregation of X*theta :',
     f          / , ' number of groups = ', I5, / ,
     f          '  x(i)*theta  failures  successes  trials' )
 820  FORMAT ( F12.6,  I10, I11, I8 )
 830  FORMAT ( /,'    obs  x(i)*theta      y(i)    counts' )
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE overlp(Ntot,YS,MS,NS,ovlap,F,S)
CCC   compute the minimal number of points from the projected
CCC   data set etaS with successes YS and MS trials
CCC   which can be removed such that the MLE does not exist
CCC   for the reduced data set in a logistic regression model
      IMPLICIT NONE
      INTEGER Ntot,NS,ovlap,i
...
CCC   Initialize F and S
   ...
   70 CONTINUE
CCC    Compute overlap
      ovlap=MIN0(F(ns),S(ns))
      ovlap=MIN0(ovlap, F(ns)-F(1), S(ns)-S(1))
      DO 80 i=2,ns-1
        ovlap=MIN0(ovlap, F(i-1)+S(ns)-S(i), S(i-1)+F(ns)-F(i))
   80 CONTINUE
      ovlap=MIN0(ovlap,F(ns-1),S(ns-1))
      RETURN
      END
